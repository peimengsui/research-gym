"""Learner scaffold for the diffusion DDPM sampling lesson.

The schedule, forward noising, timestep embedding, and tiny denoiser utilities
are carried over from earlier diffusion lessons. Your TODOs focus on the new
reverse-sampling math.
"""
...
def predict_x0_from_noise(
    ...
    # TODO: implement (x_t - sqrt(1 - alpha_bar_t) * eps) / sqrt(alpha_bar_t).
...
def ddpm_reverse_step(
    ...
    # TODO:
    # - predict noise with model(x_t, timesteps)
    # - compute the reverse mean
    # - gather posterior variance
    # - add sqrt(variance) * noise only when timestep > 0
    raise NotImplementedError
...
def ddpm_sample_loop(
    ...
    # TODO:
    # - start from torch.randn(shape)
    # - loop from T-1 down to 0
    # - call ddpm_reverse_step each time
    # - optionally return the full trajectory
    raise NotImplementedError

"""Reference solution for the diffusion DDPM sampling lesson."""
...
class DiffusionSchedule:
    """Precomputed scalar schedules for DDPM-style noising and sampling."""
...
def make_diffusion_schedule(
    ...
    """Precompute the schedule tensors used by DDPM training and sampling."""
...
class TinyNoisePredictor(nn.Module):
    ...
    def forward(self, x_t: torch.Tensor, timesteps: torch.Tensor) -> torch.Tensor:
        ...
        return self.network(torch.cat((x_t, time_embedding), dim=1))
...
def ddpm_reverse_mean(
    ...
    """Return the DDPM reverse-process mean."""

    if predicted_noise.shape != x_t.shape:
        raise ValueError("predicted_noise must have the same shape as x_t")

    beta_t = gather_by_timestep(schedule.betas.to(x_t.device), timesteps, x_t.shape)
    sqrt_one_minus_alpha_bar = gather_by_timestep(
        schedule.sqrt_one_minus_alpha_bars.to(x_t.device),
        timesteps,
        x_t.shape,
    )
    sqrt_recip_alpha = gather_by_timestep(
        schedule.sqrt_recip_alphas.to(x_t.device), timesteps, x_t.shape
    )
    return sqrt_recip_alpha * (
        x_t - beta_t / sqrt_one_minus_alpha_bar * predicted_noise
    )
...
def ddpm_reverse_step(
    ...
    """Take one stochastic DDPM reverse step.

    Returns:
        (x_previous, mean), both with shape x_t.shape.
    """
    ...
    mean = ddpm_reverse_mean(schedule, x_t, timesteps, predicted_noise)
    variance = gather_by_timestep(
        schedule.posterior_variance.to(x_t.device), timesteps, x_t.shape
    )
    if noise is None:
        noise = torch.randn(
            x_t.shape,
    ...
    nonzero_mask = (timesteps > 0).to(dtype=x_t.dtype, device=x_t.device)
    nonzero_mask = nonzero_mask.reshape((x_t.shape[0],) + (1,) * (x_t.ndim - 1))
    x_previous = mean + nonzero_mask * torch.sqrt(variance) * noise
    return x_previous, mean
...
def ddpm_sample_loop(
    ...
    """Run the full DDPM reverse sampling loop from Gaussian noise."""
